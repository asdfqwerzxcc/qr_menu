...
## ToDo
### 중복로그인 방지 기능
현재 방식은 서버세션을 사용하기 때문에 서버가 불안정하면 데이터가 날라가 작동을 하지 못할수도 있다.
이러한 방식을 막기위해 Redis를 사용하여 작동시킨다면, 데이터 손실을 방지할 수 있을것이다.

앞으로 Redis로 마이그레이션을 해야할것이다.


### 로그인 시도 제한 기능
현재 이 기능은 구현되어 있지 않아 ToDo리스트에 추가 시켜 보았다.<br>
이기능 또한 Redis를 통해 아이디별 로그인 시도 횟수 및 차단시간을 추가시켜 로그인 실패시마다 시도 횟수+1을 해주고 5인 경우 차단시간을 현재시간 +10분을 해서 저장을 해서 로그인시마다 차단시간을 확인 후 로그인하는 형태로 진행할 예정이다. 